...
def test_unit_test_list(prodtest_client: ProdtestClient) -> None:
    """unit-test-list should succeed and emit a listing header trace."""
    resp = prodtest_client.command_ok(ProdtestCommand(Cmd.UNIT_TEST_LIST))
    assert resp.traces is not None
    assert any("registered unit tests" in line for line in resp.traces)


def test_unit_test_run(prodtest_client: ProdtestClient) -> None:
    ...
    """
    resp = prodtest_client.command_ok(ProdtestCommand(Cmd.UNIT_TEST_RUN))
    assert resp.traces is not None
