...
class TropicSession:
    """A running prodtest emulator paired with its own Tropic model.

    Exposes the prodtest :attr:`client` for issuing `tropic-*` commands and,
    once the surrounding context manager has closed (so the Tropic model has
    flushed its state), :meth:`state` for inspecting the resulting model config.
    """
...
class TropicProdtest(t.Protocol):
    """Type of the ``tropic_prodtest`` fixture: a factory of :class:`TropicSession`.
