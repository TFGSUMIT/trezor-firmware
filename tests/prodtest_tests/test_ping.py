...
def test_ping_with_long_text(prodtest_client: ProdtestClient) -> None:
    """ping should echo back the provided text."""
    long_text = 512 * "longtext"
    resp = prodtest_client.command_ok(ProdtestCommand(Cmd.PING, long_text))
    assert resp.is_ok
    assert resp.args == long_text
    too_long_text = long_text + "A"
    resp = prodtest_client.command_ok(ProdtestCommand(Cmd.PING, too_long_text))
    assert resp.is_ok
    assert resp.args == long_text
