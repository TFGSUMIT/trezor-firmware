...
impl<'a> MpyBuilder<'a> {
    ...
    fn build_frozen_modules(&self, qstr_preprocessed: &Path) -> Result<PathBuf> {
        ...
        let py_src_dir = self.py_src_dir.clone();

        let compile_func = move |file: &&PathBuf| {
            Self::build_frozen_module(
                file,
                &crate_dir,
                &out_dir,
                &py_src_dir,
    ...
    fn build_frozen_module(
        ...
        let i_file = xbuild::derive_output_path(crate_dir, source, out_dir, "i");
