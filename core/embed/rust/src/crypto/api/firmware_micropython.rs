...
use trezor_app_sdk::structs::{
    ArchivedSlice, ArchivedTrezorCryptoEnum, StrSlice, TrezorCryptoResult,
};
...
extern "C" fn new_deserialize_crypto_message(
    ...
    let block = |_args: &[Obj], kwargs: &Map| {
        ...
        let result: Obj = match archived {
            ...
            ArchivedTrezorCryptoEnum::GetXpubBytes { address_n } => obj_from_dp_slice(address_n),
            ArchivedTrezorCryptoEnum::EcdsaPublicKey65 { address_n } => {
            ...
            ArchivedTrezorCryptoEnum::VerifyDerivationPath {
                address_n,
                encoded_network,
                encoded_token,
                chain_id,
...
extern "C" fn new_send_crypto_result(n_args: usize, args: *const Obj, kwargs: *mut Map) -> Obj {
    let block = |_args: &[Obj], kwargs: &Map| {
        ...
        let msg = if obj.is_str() {
            ...
            TrezorCryptoResult::Xpub(StrSlice::new(unwrap!(core::str::from_utf8(slice))))
        } else if obj.is_bytes() {
            let data = unwrap!(unsafe { crate::micropython::buffer::get_buffer(obj) });
            match data.len() {
                33 => TrezorCryptoResult::XpubBytes(unwrap!(data.try_into())),
            ...
            match tag {
                0 => TrezorCryptoResult::Signature(unwrap!(data.try_into())),
                1 => TrezorCryptoResult::EcdsaPublicKey65(unwrap!(data.try_into())),
