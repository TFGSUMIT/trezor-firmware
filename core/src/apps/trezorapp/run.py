...
_SERIVICE_CRYPTO_GET_XPUB_BYTES = const(1)
_SERVICE_CRYPTO_ECDSA_PUBLIC_KEY65 = const(2)
_SERVICE_CRYPTO_SIGN_TYPED_HASH = const(3)
_SERVICE_CRYPTO_GET_ADDRESS_MAC = const(4)
_SERVICE_CRYPTO_VERIFY_NONCE_CACHE = const(5)
_SERVICE_CRYPTO_CHECK_ADDRESS_MAC = const(6)
_SERVICE_CRYPTO_VERIFY_DERIVATION_PATH = const(7)
        ...
        if service == _SERVICE_UI:
            ...
            try:
                ...
                if message_id == _SERIVICE_CRYPTO_GET_XPUB:
                    ...
                    address_n: list[int] = obj
                    try:
                        result = [1, await _get_ecdsa_public_key_65(address_n)]
                    except Exception:
                        result = False
                ...
                elif message_id == _SERVICE_CRYPTO_VERIFY_DERIVATION_PATH:
                    assert len(obj) == 4
                    address_n: list[int] = obj[0]
                    encoded_network: bytes | None = obj[1]
                    encoded_token: bytes | None = obj[2]
                    chain_id: int | None = obj[3]
                    try:
                        await _verify_derivation_path(
            ...
            # Serialize and send the result back
            try:
                print(f"Sending crypto result: {result}")
...
async def _get_public_key_bytes(address_n: list[int]) -> bytes:
    from apps.common import coininfo, paths
    from apps.common.keychain import ForbiddenKeyPath, get_keychain
    ...
    if address_n and address_n[0] == paths.SLIP25_PURPOSE:
        # UnlockPath is required to access SLIP25 paths.
        print(f"Forbidden path: {address_n}")
        raise ForbiddenKeyPath()

    print(
        f"Getting keychain for curve: {coin.curve_name} and schema: {[paths.AlwaysMatchingSchema]}"
    )
    keychain = await get_keychain(coin.curve_name, [paths.AlwaysMatchingSchema])

    print(f"Deriving node for path: {address_n}")
    node = keychain.derive(address_n)

    print(f"Derived node for path {address_n}: {node}")
    return node.public_key()


async def _get_ecdsa_public_key_65(address_n: list[int]) -> bytes:
...
# TODO: implement
async def _verify_derivation_path(
    address_n: list[int],
    encoded_network: bytes | None,
    encoded_token: bytes | None,
    chain_id: int | None,
